...
# -------------------------------------------------------------
# 4. محرك البروكسي (Proxy Engine) - تم تعديل استبدال الامتداد هنا
# -------------------------------------------------------------
@app.route('/proxy')
def proxy_stream():
    host = flask_session.get('host')
    ...
    # هنا التعديل الذكي: إذا كان نوع البث مباشر (live) والامتداد ts، نقوم بتحويله تلقائياً إلى m3u8
    if stream_type == 'live' and ext == 'ts':
        ext = 'm3u8'

    # بناء رابط البث المباشر الموجه للسيرفر الأصلي بالمظهر الجديد (m3u8)
    ...
    try:
        ...
        # نقوم بتمرير الـ Content-Type الصحيح الذي يعود به السيرفر لقراءة الـ m3u8 بأمان
                ...
                for cat in cats:
                    ...
                    if isinstance(streams, list):
                        for item in streams:
                            ...
                            if s_id:
                                # قمنا بتعديل امتداد التحميل لملف الـ M3U أيضاً ليعتمد m3u8 للبث المباشر
...
if __name__ == '__main__':
    app.run(debug=True, host='0.0.0.0', port=5000)        .tabs { display: flex; background: #161226; border-bottom: 1px solid rgba(255,255,255,0.05); }
        .tab { flex: 1; padding: 15px; text-align: center; cursor: pointer; font-weight: bold; color: #8a8594; }
        .tab.active { color: #00f2fe; background: #110e1c; border-bottom: 2px solid #00f2fe; }
        .items-list { padding: 10px; overflow-y: auto; flex: 1; }
        .item-row { display: flex; align-items: center; padding: 12px 15px; border-bottom: 1px solid rgba(255,255,255,0.02); cursor: pointer; border-radius: 8px; margin-bottom: 4px; transition: 0.2s; }
